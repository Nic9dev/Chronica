...
- **削除のみ**: 不要な記憶は削除で対応
- **トークン可視化**: トークン上限（デフォルト 500,000、最大 5,000,000）をサイドバーで変更可能。値は `data/curation_ui_settings.json` に保存され、ページ更新後も維持される
...
- [ ] 統計情報（記憶数の推移グラフ等）
- [ ] システムプロンプトの継続的微調整
- [ ] **記憶の可視化階層設定（Memory Visibility Layers）**
  - グローバル記憶：どのプロジェクト・スレッドからでも参照できる共有記憶層
  - プロジェクト記憶：特定プロジェクト内でのみ参照できる記憶（現状の project スレッド）
  - スレッド記憶：特定スレッド内でのみ参照できる一時的な記憶（現状の thread_id 指定）
  - compose_opening 時に参照する階層を設定できる仕組み
  - クロスプロジェクト検索（グローバル記憶を複数プロジェクトから横断参照）
  - 背景：現状、project スレッドを複数作成して部屋ごとに使い分けるユースケースでは、プロジェクト間で共有できる個人記憶層が存在しない。製作者本人の実体験から生まれた要件。
